...
class NavigationPage {
  constructor() {
    ...
    this.lastScroll = 0;
    let self = this;
    jQuery(".nav-tab").click(function () {G
  ...
  onTabClick(event, element) {
    ...
    let scrollTop =
      jQuery(element.attr("href")).offset().top - this.tabContainerHeight + 1;
  ...
  checkHeaderPosition() {
    ...
    if (jQuery(window).scrollTop() > headerHeight) {
      jQuery(".nav-container").addClass("nav-container--scrolled");
    } else {
      jQuery(".nav-container").removeClass("nav-container--scrolled");
    }
    let offset =
      ...
      headerHeight;
    if (
      jQuery(window).scrollTop() > this.lastScroll &&
      jQuery(window).scrollTop() > offset
    ) {
      jQuery(".nav-container").addClass("nav-container--move-up");
      jQuery(".nav-container").removeClass("nav-container--top-first");
      jQuery(".nav-container").addClass("nav-container--top-second");
    } else if (
      jQuery(window).scrollTop() < this.lastScroll &&
      jQuery(window).scrollTop() > offset
    ) {
      jQuery(".nav-container").removeClass("nav-container--move-up");
      jQuery(".nav-container").removeClass("nav-container--top-second");
      jQuery(".nav-container-container").addClass("nav-container--top-first");
    } else {
      jQuery(".nav-container").removeClass("nav-container--move-up");
      jQuery(".nav-container").removeClass("nav-container--top-first");
      jQuery(".nav-container").removeClass("nav-container--top-second");
  ...
  findCurrentTabSelector(element) {
    ...
    jQuery(".nav-tab").each(function () {
      ...
      let offsetBottom =
        jQuery(id).offset().top + jQuery(id).height() - self.tabContainerHeight;
      if (
        jQuery(window).scrollTop() > offsetTop &&
        jQuery(window).scrollTop() < offsetBottom
      ) {
        newCurrentId = id;
  ...
  setSliderCss() {
    ...
    jQuery(".nav-tab-slider").css("width", width);
    jQuery(".nav-tab-slider").css("left", left);
  }
}
